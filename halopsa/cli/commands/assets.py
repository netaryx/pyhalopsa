"""Assets command with typed filters."""
...
LIST_COLUMNS = ["id", "inventory_number", "key_field", "client_name", "assettype_name", "status", "devicename"]
DETAIL_COLUMNS = [
    "id", "inventory_number", "key_field", "key_field2", "key_field3",
    "client_id", "client_name", "site_id", "site_name", "username",
    "assettype_id", "assettype_name", "status", "devicename", "serialnumber",
    "assettag", "warranty_end", "supplier_id", "supplier_name",
...
def list_cmd(
    ...
    assettype_id: Optional[int] = typer.Option(None, "--assettype-id", help="Filter by asset type ID."),
    ...
    try:
        ...
        if assettype_id is not None:
            kwargs["assettype_id"] = assettype_id
        if search is not None:
            kwargs["search"] = search

        if all_pages:
            items_raw = list(client.assets.list_all(page_size=page_size, **kwargs))
            items = [_model_to_dict(i) for i in items_raw]
        else:
            result = client.assets.list(page_size=page_size, page_no=page_no, **kwargs)
            items = [_model_to_dict(i) for i in result.items]

        if state.json_output:
            print_json(items)
        else:
            print_table(items, LIST_COLUMNS, title="Assets")
    except SystemExit:
        raise
